...
class WeiTopoNet(pl.LightningModule):
    # Takes input size [60, 201]
    def __init__(self):
        super().__init__()
        self.c1 = self.conv(in_channels=60)
        self.c2 = self.conv()
        self.pool = nn.AvgPool1d(kernel_size=2, stride=2)
        self.dropout = nn.Dropout1d(0.25)

        self.c3 = self.conv(in_channels=128, out_channels=256)
        self.c4 = self.conv(in_channels=256, out_channels=256)

        self.d1 = self.dense(12800, 2048)
        self.d2 = self.dense(2048, 2048)
        self.d3 = self.dense(2048, 2048, activation=nn.ReLU())
        self.dropout2 = nn.Dropout1d(0.25)
